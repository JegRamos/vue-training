  ...
  <div>
    ...
    <v-card flat style="border: solid black 1px">
      <v-card-title>
        <v-layout row>
          <v-flex>
            <h1 class="text-xs-center heading">EDIT FACILITY</h1>
          </v-flex>
        </v-layout>
      </v-card-title>
      ...
      <v-card-text>
        <v-form>
          <v-container>
            ...
            <v-layout row>
              <v-flex>
                <v-btn @click="editFacility" depressed fab dark color="mt-2 success right">
...
export default {
  ...
  methods: {
    fetchFacility () {
      this.$http.get(`http://127.0.0.1:3000/facilities/${this.$route.params.id}`)
        .then(res => {
          return res.json()
        })
        .then(data => {
    ...
    editFacility () {
      this.$http.patch(`http://127.0.0.1:3000/facilities/${this.$route.params.id}`, this.facility)
        .then(res => {
          console.log({ res })
